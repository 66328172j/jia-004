...
CREATE TABLE IF NOT EXISTS t_samp_lot (
  ...
  grade_code varchar(16) DEFAULT NULL COMMENT '质量等级字码（质量定等回写）',
  grade_name varchar(64) DEFAULT NULL COMMENT '质量等级名称（质量定等回写）',
...
-- 已按早期版本建过 t_samp_lot 的库，补质量定等回写两列（新库无需执行）：
-- ALTER TABLE t_samp_lot ADD COLUMN grade_code varchar(16) DEFAULT NULL COMMENT '质量等级字码（质量定等回写）' AFTER conclude;
-- ALTER TABLE t_samp_lot ADD COLUMN grade_name varchar(64) DEFAULT NULL COMMENT '质量等级名称（质量定等回写）' AFTER grade_code;
...
-- 移动端扫码查询记录（客户扫码/手输批号查询，无论命中与否都落一条）
CREATE TABLE IF NOT EXISTS t_samp_query_log (
  id varchar(32) NOT NULL COMMENT '主键',
  lot_no varchar(64) NOT NULL COMMENT '查询的批号',
  lot_id bigint DEFAULT NULL COMMENT '命中的检验批ID（未命中为空）',
  query_source varchar(32) DEFAULT 'H5' COMMENT '查询来源 H5/WECHAT/APP',
  query_time datetime DEFAULT NULL COMMENT '查询时间',
  result_flag int DEFAULT '0' COMMENT '是否命中 0未命中 1命中',
  create_time datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (id),
  KEY idx_sql_lot_no (lot_no),
  KEY idx_sql_time (query_time)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='移动端扫码查询记录';
