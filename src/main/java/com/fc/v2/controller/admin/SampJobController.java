...
public class SampJobController extends BaseController {
    ...
    public AjaxResult run(@PathVariable("id") String id) {
        TSampJobLog jobLog;
        try {
            jobLog = tSampJobService.runOverdueScan(id);
        } catch (IllegalStateException e) {
            // 同一任务正在执行中（重复点击/与定时触发撞车），本次触发被拒绝
            return error(e.getMessage());
        }
        if (jobLog == null) {
            return error("任务不存在或已停用，未执行");
        }
        // 执行结果以任务日志状态为准：失败要在页面上能看出失败，而不是笼统提示"执行完成"
        if (jobLog.getStatus() != null && jobLog.getStatus() == 1) {
            String msg = StringUtils.isNotEmpty(jobLog.getErrorMsg()) ? jobLog.getErrorMsg() : "详见任务日志";
            // 错误信息最长 2000 字，提示里只带前 200 字，完整内容到任务日志里看
            if (msg.length() > 200) {
                msg = msg.substring(0, 200) + "……";
            }
            return error("执行失败：" + msg);
        }
