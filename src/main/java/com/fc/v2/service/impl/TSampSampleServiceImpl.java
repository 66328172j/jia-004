...
public class TSampSampleServiceImpl extends ServiceImpl<TSampSampleMapper, TSampSample> implements ITSampSampleService {
    ...
    /**
     * 按当前样本检测记录刷新检验批汇总与状态：
     * 0待抽样（无记录）→ 1抽样中（记录不足额）→ 2待判定（录满，瞬时）→ 3已判定（按Ac/Re自动判定）；
    ...
    private void refreshLotSummary(Long lotId) {
        ...
        if (planned > 0 && count >= planned) {
            // 样本录满：按抽样方案的接收数/拒收数自动判定，判定后流转到已判定；
            // 落在 Ac/Re 间隙（个别加严方案）时停在待判定
            autoJudge(wrapper, lot, defect);
            return;
        }
    ...
    /**
     * 按一次抽样方案判定：不合格样本数 d ≤ Ac 接收（合格），d ≥ Re 拒收（不合格）；
     * 落在 Ac/Re 间隙（个别加严方案）时保留待判定状态
     *
     * @param wrapper 已带汇总字段的更新条件
     * @param lot     检验批
     * @param defect  不合格样本数
     */
    private void autoJudge(UpdateWrapper<TSampLot> wrapper, TSampLot lot, int defect) {
        Integer accept = lot.getAcceptCount();
        Integer reject = lot.getRejectCount();
        if (accept != null && defect <= accept) {
            wrapper.set("status", 3).set("conclude", "合格");
        } else if (reject != null && defect >= reject) {
            wrapper.set("status", 3).set("conclude", "不合格");
        } else {
            wrapper.set("status", 2).set("conclude", null);
        }
        tSampLotMapper.update(null, wrapper);
    }
